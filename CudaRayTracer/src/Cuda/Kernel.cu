...
__global__ void kernel(unsigned int* pos, unsigned int width, unsigned int height, const unsigned int samples_per_pixel, const unsigned int max_depth, hittable** world, curandState* rand_state, InputStruct inputs)
{
    ...
    unsigned int pixel_index = (y * width + x) * 4;

    curandState local_rand_state = rand_state[pixel_index / 4];
    ...
    float3 secondPlanePos = (sizeFarPlane * distFromCenter) + origin + (distFarPlane * forwardV);
    ...
    rand_state[pixel_index / 4] = local_rand_state;
    ...
    // write output vertex
    pos[pixel_index] = col.x;
    pos[pixel_index + 1] = col.y;
    pos[pixel_index + 2] = col.z;
}
